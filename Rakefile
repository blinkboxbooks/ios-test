...
namespace :ios do
  ...
  task :compile_app do
    ...
    if !Dir[@conf['project']['src_dir']].empty?
      Dir.chdir(@conf['project']['src_dir']) do
        ret = system("xcodebuild -scheme BBBApp -configuration Debug -target BBBApp -sdk iphonesimulator7.1")
        if !ret 
          raise "Build failed with error"
        end
  ...
  task :install_app do
    if !system("ios-sim --help 2>/dev/null") 
      raise "Warning ios-sim is not installed with brew. Please run 'rake calabash:environment_install'"
    end
